...
def export_article_cmd(ctx, slug):
    """Export a single article with full context (backlinks, sources, related)."""
...
def export_tag_cmd(ctx, tag):
    ...
    import json
    result = export_by_tag(tag, ctx.obj["base_dir"])
    click.echo(json.dumps(result, indent=2, ensure_ascii=False))


@export.command("graph")
@click.argument("slug")
@click.option("--depth", type=int, default=2, help="Traversal depth (default 2)")
@click.pass_context
def export_graph_cmd(ctx, slug, depth):
    """Export article subgraph (N levels of connections)."""
    from .export import export_graph
    import json
    result = export_graph(slug, depth, ctx.obj["base_dir"])
    click.echo(json.dumps(result, indent=2, ensure_ascii=False))
